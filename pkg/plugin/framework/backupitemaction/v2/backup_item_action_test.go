...
func TestBackupItemActionGRPCServerExecute(t *testing.T) {
	...
	tests := []struct {
		...
		implUpdateAdditionalItemsAfterOperation bool
		implError                               error
		expectError                             bool
		skipMock                                bool
	}{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !test.skipMock {
				itemAction.On("Execute", &validItemObject, &validBackupObject).Return(test.implUpdatedItem, test.implAdditionalItems, test.implOperationID, test.implUpdateAdditionalItemsAfterOperation, test.implError)
			}
